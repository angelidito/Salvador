...
REM 		   la fecha de la última copia y lo borra.
IF exist ..\Fechas\%volumenParaLimpiar%\nul (echo Va a borrar: && rd /S ..\Fechas\%volumenParaLimpiar% && echo Fecha de última copia eliminada) ELSE (echo No se encuentra fecha copia de este volumen. && echo. && echo Es posible que haya sido borada o que nunca se haya realizado una copia. && echo. && pause && CLS)
...
REM Compueba que estén bien los datos
:esCorrectoDestino
CLS
echo.
...
CLS
echo.
echo  Ruta donde está la copia: %volumenDondeEstaLaCopia%:\%rutaParaLimpiar%\%volumenParaLimpiar%
echo.
